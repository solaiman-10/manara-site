/* ============================================
   المنارة — واجهة خلفية مشتركة (Netlify Function)
   حسابات حقيقية (بريد إلكتروني أو جوال سعودي + OTP)
   + حصص + سجل مراجعة المالك.
   التخزين عبر Netlify Blobs. كلمات المرور تُخزَّن
   مشفرة (scrypt + salt).

   بيئة اختيارية:
   - MANARA_OWNER_EMAIL / MANARA_OWNER_PASSWORD
...
const STORE_NAME = "manara-data";
const OWNER_EMAIL = process.env.MANARA_OWNER_EMAIL || "admin@manara.edu";
const OWNER_PASSWORD = process.env.MANARA_OWNER_PASSWORD || "admin123";
...
async function route(event) {
  ...
  if (!store) {
    return action === "ping"
      ? ok({site: "manara", store: STORE_NAME, demo: true, error: "التخزين السحابي غير متاح في بيئة التشغيل — فعّل Netlify Blobs من إعدادات الموقع"})
  ...
  switch (action) {
    ...
    case "ping":
      return ok({ site: "manara", store: STORE_NAME });
    ...
    case "sendOtp": {
      ...
      let sent = {};
      try {
        sent = await sendSms(phone, "رمز التحقق الخاص بك في المنارة هو: " + code);
      } catch (e) {
    ...
    case "signup": {
      ...
      const account = {
        ...
        provider: body.provider === "google" || body.provider === "apple" ? body.provider : "email",
        certName,
    ...
    case "approveTeacher": {
      ...
      if ((!email && !phone) || !status) return fail("بيانات غير صحيحة");
      const accounts = await readJSON(store, "accounts.json", []);
